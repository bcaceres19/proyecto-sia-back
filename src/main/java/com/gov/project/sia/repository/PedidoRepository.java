...
public interface PedidoRepository extends JpaRepository<PedidoEntity, String> {
    ...
    @Query(value = """
            ...
            """, nativeQuery = true)
    List<Object[]> buscarDataReporte(@Param("codigoPedido") String codigoPedido);
    ...
    @Query(value = """
             ...
             from sia.pedido p inner join
            sia.venta v on p.id_venta_fk = v.id_venta inner join sia.usuario u on
             p.id_usuario_fk = u.id_usuario where (:idUsuario is null or u.id_usuario = :idUsuario)
             and (:tipo = 'eliminada' or p.estado_pedido = 'A') and (:tipo = 'aceptada' or p.estado_pedido='EU')
             """, nativeQuery = true)
    List<Object[]> buscarPedidosCompletados(@Param("idUsuario") Integer idUsuario, @Param("tipo") String tipo);
